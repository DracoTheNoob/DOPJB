...
class App:
    def __init__(self):
        ...
        self.background: Surface = pygame.transform.scale(pygame.image.load(f'src/sprites/background-{self.enemy.role}.png'), size=(WINDOW_WIDTH, WINDOW_HEIGHT))
    ...
    def init(self):
        ...
        for i, players in enumerate([self.back_players, self.front_players]):
            ...
            for j, player in enumerate(players):
                pos: tuple[int, int] = (
                    int(MARGIN + i*(PLAYER_SIZE + MARGIN + 40) + PLAYER_SIZE/2),
                    int(WINDOW_HEIGHT / 2 - PLAYER_SIZE / 2 - (n // 2 - j)*(PLAYER_SIZE + MARGIN) + (0 if odd else (PLAYER_SIZE + MARGIN)/2) + PLAYER_SIZE/2)
                )
        ...
        self.end_tick = max([a.start + (a.duration if a.duration is not None else 0) for a in self.animations]) - 5 * TPS
    ...
    def render(self):
        self.draw.image(self.background, 0, 0)

        # Draw players
        for i, player in enumerate(self.players):
            self.draw.entity(PLAYER_POS[i][0], PLAYER_POS[i][1], player.role, tick=self.ticks)
            self.draw.health_bar(PLAYER_POS[i][0] + PLAYER_SIZE + 10 - PLAYER_SIZE/2, PLAYER_POS[i][1] - PLAYER_SIZE/2, player, height=PLAYER_SIZE)

        # Draw the enemy
        self.draw.square((100, 50, 150), ENEMY_X, ENEMY_Y, ENEMY_SIZE)
        # Draw the enemy's health bar
        self.draw.health_bar(ENEMY_X - 40, ENEMY_Y, self.enemy, height=ENEMY_SIZE)
        ...
        if self.ticks >= self.end_tick + 6*TPS:
    ...
    def update(self):
        ...
        if self.ticks == self.end_tick:
            self.battle_music.fadeout(5000)
